...
import cross
cross_table = cross.build_cross_table(Path("charts"))
cross.plot_cross_table(
    cross_table,
...
```{python}
#| label: charts-vs-score
#| fig-cap: "Charts vs Score"
merged = cross.charts_and_scores("./tables/results.csv", cross_table)
cross.scatter_charts_scores(merged)
```
...
After looking at the heatmap above, I thought that maybe Spotify’s country charts can let us approximate how much listeners embrace the contest in each country, not just their own entry. I counted, for every country, how many of the 26 finalist songs entered the local top‑200 in the first post‑Eurovision snapshot to create a “score”:

```{python}
#| label: eurovibe-bars
#| fig-cap: "Eurovibe bars"
import matplotlib.pyplot as plt
eu_counts = cross_table.notna().sum(axis=1).sort_values(ascending=False)
eu_counts.plot(kind="bar")
...
| -------------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **Heatwave**   | 20 – 26 finalist songs charted | Austria, Estonia, Finland, Germany, Iceland, Latvia, Lithuania, Luxembourg, Netherlands, Norway, Poland, Sweden, Switzerland | These places, at least for this week, seem to breathe Eurovision. The Nordics and Baltics cluster is quite clear here but they are not alone. |
...
A month passed, the world moved on. A few days ago I heard my son singing to himself a familiar tune “Espresso Macchiato …”. I was curious and asked him where did he pick it up. He told me that it’s the song everyone is singing in his school. So, I went back to the charts. I repeated the scrape I ran four weeks ago. Most tracks had slipped away, but some endured. Here is my second snapshot:

```{python}
#| label: cross-later
#| fig-cap: "One Month Later – heatmap"
cross_table_later = cross.build_cross_table(Path("charts2106"))
cross.plot_cross_table(
    cross_table_later,
    label="Best chart position",
    title="Chart positions one month later",
    reverse=True,
)
```

```{python}
#| label: bar-count-later
#| fig-cap: "One Month Later – bars"
cross.plot_count(cross_table_later)
```

Three observations emerge:

1. **Estonia holds the crown for staying power.** “Espresso Macchiato” stayed on most charts, keeping 16 of its initial 29 placements – a retention rate of 55%. That might suggest organic replay rather than just a momentary meme. If you want, Tommy Cash is the true empirical winner of this year, or at least the true hit‑maker.
...
*\[Code available [here](https://github.com/menisadi/eurovision2025)]*
