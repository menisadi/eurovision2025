...
```bash
# Fetch chart snapshots (saved under charts/YYYYMMDD/)
...
1. **`fetch.py`** — scrapes Kworb Spotify daily charts for all countries; `--out-dir` controls output folder
2. **`cross.py`** — builds a cross-table from a chart folder + results CSV: rows = chart country, columns = Eurovision country, values = best chart position. Contains all plotting helpers (`plot_cross_table`, `scatter_charts_scores`, `plot_count`, etc.)
...
- **`tables/2026/`** — 2026 data: `results.csv` (final scores with Place, Country, Song, Artist, Points, Public, Jury, Running)
- **`tables/2025/`** — archived 2025 tables (results, jury, public, cross-tables)
- **`charts/20250517/`**, **`charts/20250621/`** — 2025 chart snapshots (week 1 and ~1 month later)
- **`charts/20260517/`**, **`charts/20260518/`**, **`charts/20260519/`** — 2026 chart snapshots
...
For each new snapshot: run `fetch.py --out-dir charts/YYYYMMDD`, then pass `--chart-dir charts/YYYYMMDD` to `generate_charts.py`. Artist matching is done on lowercased name — check for mismatches if chart counts look unexpectedly low.
